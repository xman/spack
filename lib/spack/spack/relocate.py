...
def file_is_relocatable(filepath, paths_to_relocate=None):
    """Returns True if the file passed as argument is relocatable.

    Args:
        filepath: absolute path of the file to be analyzed
    ...
    if not os.path.isabs(filepath):
        raise ValueError('{0} is not an absolute path'.format(filepath))

    strings = executable.Executable('strings')

    # Remove the RPATHS from the strings in the executable
    set_of_strings = set(strings(filepath, output=str).split())

    m_type, m_subtype = mime_type(filepath)
    if m_type == 'application':
        tty.debug('{0},{1}'.format(m_type, m_subtype))

    if platform.system().lower() == 'linux':
        if m_subtype == 'x-executable' or m_subtype == 'x-sharedlib':
            rpaths = ':'.join(_elf_rpaths_for(filepath))
    ...
    for path_to_relocate in paths_to_relocate:
        if any(path_to_relocate in x for x in set_of_strings):
            ...
            tty.debug(msg.format(path_to_relocate, filepath))
            return False
